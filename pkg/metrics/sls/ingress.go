...
func (ss *SLSMetricSource) getSLSIngressPredictQuery(params *SLSIngressParams, metricName string) (begin int64, end int64, query string) {
	...
	case SLS_INGRESS_QPM:
		...
		begin = (now / 60 - 10) * 60
		var querySearch string = fmt.Sprintf(`proxy_upstream_name: '%v' or proxy_alternative_upstream_name: '%v'`, params.TargetRouter, params.TargetRouter)
	...
	case SLS_INGRESS_PREDICT:
		...
		var querySearch string = fmt.Sprintf(`__tag__:__model_type__: predict and '%v'`, params.TargetRouter)
